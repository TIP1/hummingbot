...
def public_rest_url(path_url: str, domain: str = CONSTANTS.DEFAULT_DOMAIN) -> str:
    ...
    :param domain: the Deribit domain to connect to ("com" or "us"). The default value is "com"
        ...
        if response.status != 200:
            if return_err:
                ...
                if error_response is not None and "code" in error_response and "msg" in error_response:
                    raise IOError(f"The request to Deribit failed. Error: {error_response}. Request: {request}")
                else:
