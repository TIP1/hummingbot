...
import hummingbot.connector.exchange.deribit.deribit_web_utils as web_utils
from hummingbot.connector.client_order_tracker import ClientOrderTracker
from hummingbot.connector.exchange.deribit import deribit_utils
from hummingbot.connector.exchange.deribit.deribit_api_order_book_data_source import DeribitAPIOrderBookDataSource
from hummingbot.connector.exchange.deribit.deribit_api_user_stream_data_source import DeribitAPIUserStreamDataSource
...
class DeribitExchange(ExchangeBase):
    ...
    def __init__(self,
        ...
        self._domain = domain
        self._deribit_time_synchronizer = TimeSynchronizer()
        super().__init__()
        self._trading_required = trading_required
        self._auth = DeribitAuth(
            api_key=deribit_api_key,
            secret_key=deribit_api_secret,
            time_provider=self._deribit_time_synchronizer)
        self._throttler = AsyncThrottler(CONSTANTS.RATE_LIMITS)
        self._api_factory = web_utils.build_api_factory(
            throttler=self._throttler,
            time_synchronizer=self._deribit_time_synchronizer,
            domain=self._domain,
            auth=self._auth)
        self._rest_assistant = None
        self._order_book_tracker = OrderBookTracker(
            data_source=DeribitAPIOrderBookDataSource(
        ...
        self._user_stream_tracker = UserStreamTracker(
            data_source=DeribitAPIUserStreamDataSource(
                auth=self._auth,
                domain=self._domain,
                throttler=self._throttler,
                api_factory=self._api_factory,
        ...
        self._last_trades_poll_deribit_timestamp = 0
    ...
    def name(self) -> str:
        if self._domain == "com":
            ...
            return f"deribit_{self._domain}"
    ...
    def status_dict(self) -> Dict[str, bool]:
        ...
        """
        return {
            "symbols_mapping_initialized": DeribitAPIOrderBookDataSource.trading_pair_symbol_map_ready(
                domain=self._domain),
    ...
    @staticmethod
    def deribit_order_type(order_type: OrderType) -> str:
        return order_type.name.upper()

    @staticmethod
    def to_hb_order_type(deribit_type: str) -> OrderType:
        return OrderType[deribit_type]
    ...
    async def cancel_all(self, timeout_seconds: float) -> List[CancellationResult]:
        ...
        try:
            ...
            self.logger().network(
                ...
                exc_info=True,
                app_warning_msg="Failed to cancel order with Deribit. Check API key and network connection."
            )
    ...
    async def _create_order(self,
        ...
        price_str = f"{price:f}"
        type_str = DeribitExchange.deribit_order_type(order_type)
        side_str = CONSTANTS.SIDE_BUY if trade_type is TradeType.BUY else CONSTANTS.SIDE_SELL
        symbol = await DeribitAPIOrderBookDataSource.exchange_symbol_associated_to_pair(
            trading_pair=trading_pair,
            domain=self._domain,
            api_factory=self._api_factory,
            throttler=self._throttler,
        ...
        try:
            ...
            self.logger().network(
                f"Error submitting {side_str} {type_str} order to Deribit for "
    ...
    async def _status_polling_loop(self):
        ...
        It also updates the time synchronizer. This is necessary because Deribit require the time of the client to be
        ...
        while True:
            try:
                ...
                self.logger().network("Unexpected error while fetching account updates.", exc_info=True,
                                      app_warning_msg="Could not fetch account updates from Deribit. "
        ...
        while True:
            try:
                ...
                self.logger().network("Unexpected error while fetching trading rules.", exc_info=True,
                                      app_warning_msg="Could not fetch new trading rules from Deribit. "
    ...
    async def _format_trading_rules(self, exchange_info_dict: Dict[str, Any]) -> List[TradingRule]:
        ...
        retval = []
        for rule in filter(deribit_utils.is_exchange_information_valid, trading_pair_rules):
            try:
                trading_pair = await DeribitAPIOrderBookDataSource.trading_pair_associated_to_exchange_symbol(
    ...
    async def _user_stream_event_listener(self):
        ...
        async for event_message in self._iter_user_event_queue():
            try:
                ...
                # Refer to https://github.com/deribit-exchange/deribit-official-api-docs/blob/master/user-data-stream.md
                # As per the order update section in Deribit the ID of the order being cancelled is under the "C" key
    ...
    async def _update_order_fills_from_trades(self):
        ...
        in case Deribit's user stream events are not working.
        NOTE: It is not required to copy this functionality in other connectors.
        This is separated from _update_order_status which only updates the order status without producing filled
        events, since Deribit's get order endpoint does not return trade IDs.
        ...
        if (long_interval_current_tick > long_interval_last_tick
            ...
            query_time = int(self._last_trades_poll_deribit_timestamp * 1e3)
            self._last_trades_poll_deribit_timestamp = self._deribit_time_synchronizer.time()
    ...
    async def _update_order_status(self):
        # This is intended to be a backup measure to close straggler orders, in case Deribit's user stream events
        # are not working.
        # The minimum poll interval for order status is 10 seconds.
    ...
    async def _update_time_synchronizer(self):
        try:
            await self._deribit_time_synchronizer.update_server_time_offset_with_time_provider(
            ...
            self.logger().exception("Error requesting time from Deribit server")
            raise
