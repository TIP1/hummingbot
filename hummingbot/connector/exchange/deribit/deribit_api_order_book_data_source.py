...
from hummingbot.connector.exchange.deribit import deribit_web_utils as web_utils
...
class DeribitAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def get_last_traded_prices(cls,
        ...
        :param domain: which Deribit domain we are connecting to (the default value is 'com')
    ...
    async def get_all_mid_prices(domain: str = CONSTANTS.DEFAULT_DOMAIN) -> Dict[str, Decimal]:
        ...
        for record in resp_json:
            try:
                pair = await DeribitAPIOrderBookDataSource.trading_pair_associated_to_exchange_symbol(
    ...
    async def exchange_symbol_associated_to_pair(
        ...
        """
        symbol_map = await DeribitAPIOrderBookDataSource.trading_pair_symbol_map(
            domain=domain,
            api_factory=api_factory,
            throttler=throttler,
            time_synchronizer=time_synchronizer)
    ...
    async def fetch_trading_pairs(
        ...
        """
        mapping = await DeribitAPIOrderBookDataSource.trading_pair_symbol_map(
            domain=domain,
            throttler=throttler,
            api_factory=api_factory,
            time_synchronizer=time_synchronizer,
        ...
        while True:
            try:
                ...
                trading_pair = await DeribitAPIOrderBookDataSource.trading_pair_associated_to_exchange_symbol(
                    ...
                    throttler=self._throttler,
                    time_synchronizer=self._time_synchronizer)
                trade_msg: OrderBookMessage = DeribitOrderBook.trade_message_from_exchange(
    ...
    async def _init_trading_pair_symbols(
        ...
        try:
            ...
            for symbol_data in filter(deribit_utils.is_exchange_information_valid, data["symbols"]):
